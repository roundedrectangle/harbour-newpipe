...
MEMCHECK=$(ssh ${COMPILEHOST} 'free -g | tail -n 2 | cut -d " +" -F 2 | echo $(awk "{sum+=\$1} END {print sum >= 16}")')
...
ssh ${COMPILEHOST} "if [ ! -d \"${COMPILEHOST_GRAAL}\" ]; then cd ${COMPILEHOST_BASE}; mkdir graalvm; cd graalvm; curl ${GRAALVM_DOWNLOAD} -o graalvm.tar.gz; tar -xf graalvm.tar.gz; else echo Already installed; fi"

# Trqnsfer using rsync
echo "Transfering code to phone"
...
echo "Starting build"
ssh ${COMPILEHOST} "cd ${COMPILEHOST_WORKSPACE}; GRAALVM_HOME=${COMPILEHOST_GRAAL} JAVA_HOME=${COMPILEHOST_GRAAL} ./gradlew nativeCompile" --console verbose
